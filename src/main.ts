...
function getFileStat(
  filePath: string,
): { size: number; mtime: string; mtimeMs: number } | null {
  try {
...
function buildMetaBlock(
  ...
  const allTags = [...new Set([...defaultTags, ext, "external-bridge"])]
    .map((t) => `  - ${t}`)
    .join("\n");
...
function mergeUpdatedContent(
  ...
  const preservedUserContent =
    existingContent.split(USER_CONTENT_START)[1]?.split(USER_CONTENT_END)[0] ??
    "";
...
class FolderWatcher {
  ...
  start() {
    ...
    // Seed known files
    const initial = getFilesRecursive(
      this.dir,
      this.extensions,
      this.recursive,
    );
  ...
  private startPolling() {
    this.pollInterval = setInterval(() => {
      const current = getFilesRecursive(
        this.dir,
        this.extensions,
        this.recursive,
      );
...
export default class ExternalBridgePlugin extends Plugin {
      ...
      async (event, filePath) => {
        ...
        if (event === "unlink") {
          ...
          if (file instanceof TFile) {
            ...
            new Notice(
              `External Bridge: removed placeholder for deleted file "${path.basename(filePath)}"`,
            );
          }
        } else {
          // add or change
          ...
          new Notice(
            `External Bridge: ${event === "add" ? "created" : "updated"} "${path.basename(filePath)}"`,
          );
        }
      },
    );
  ...
  externalPathToVaultPath(
    bridge: BridgedFolder,
    externalFile: string,
  ): string | null {
    const ext = path.extname(externalFile).slice(1).toLowerCase();
    if (
      bridge.includedExtensions.length > 0 &&
      !bridge.includedExtensions.includes(ext)
    )
      return null;
  ...
  async upsertPlaceholder(
    ...
    if (!existing) {
      const content = buildFullPlaceholder(
        externalFile,
        bridge,
    ...
    const storedMtimeMatch = existingContent.match(
      /external-mtime-ms:\s*(\d+)/,
    );
    ...
    const merged = mergeUpdatedContent(
      existingContent,
      externalFile,
      bridge,
      this.settings.defaultTags,
  ...
  async syncBridge(
    ...
    const externalFiles = getFilesRecursive(
      bridge.externalPath,
      bridge.includedExtensions,
      bridge.recursive,
    );
    ...
    for (const externalFile of externalFiles) {
      ...
      const result = await this.upsertPlaceholder(
        bridge,
        externalFile,
        vaultPath,
      );
  ...
  renderBridgeCard(container: HTMLElement, bridge: BridgedFolder) {
    ...
    syncBtn.onclick = async () => {
      ...
      try {
        ...
        new Notice(
          `"${bridge.label}": ${result.created} created, ${result.updated} updated, ${result.skipped} skipped, ${result.removed} removed.`,
        );
        this.render();
      } catch (e) {
    ...
    removeBtn.onclick = async () => {
      ...
      await this.plugin.saveSettings();
      new Notice(
        `Bridge "${bridge.label}" removed. Placeholder files remain in your vault.`,
      );
      this.render();
    };
  }
...
class AddBridgeModal extends Modal {
  ...
  onOpen() {
    ...
    new Setting(contentEl)
      ...
      .addText((t) =>
        t.setPlaceholder("My PDF Notes").onChange((v) => (this.label = v)),
      );
    ...
    new Setting(contentEl)
      ...
      .addText((t) =>
        t
          ...
          .onChange((v) => (this.externalPath = v)),
      );
    ...
    new Setting(contentEl)
      ...
      .addText((t) =>
        t.setValue(this.vaultFolder).onChange((v) => (this.vaultFolder = v)),
      );

    new Setting(contentEl)
      .setName("File types")
      .setDesc("Which file types to create placeholders for");
    ...
    new Setting(contentEl)
      ...
      .addToggle((t) =>
        t.setValue(this.recursive).onChange((v) => (this.recursive = v)),
      );

    new Setting(contentEl)
      .setName("Enable file watcher")
      .setDesc(
        "Automatically update placeholders when external files are added, changed, or deleted",
      )
      .addToggle((t) =>
        t.setValue(this.watchEnabled).onChange((v) => (this.watchEnabled = v)),
      );
    ...
    saveBtn.onclick = async () => {
      ...
      this.onSave();
      new Notice(
        `Bridge "${this.label}" added! Run Sync to create placeholders.`,
      );
    };
...
class ExternalBridgeSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "Comma-separated tags added to all placeholder files (e.g. source/external, review)",
      )
      .addText((t) =>
        t
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "How long to wait after a file change before updating the placeholder (default: 2000)",
      )
      .addText((t) =>
        t
    ...
    if (this.plugin.settings.bridges.length === 0) {
      ...
      for (const bridge of this.plugin.settings.bridges) {
        new Setting(containerEl)
          ...
          .setDesc(
            `${bridge.externalPath} → ${bridge.vaultFolder}  |  Watch: ${bridge.watchEnabled ? "on" : "off"}`,
          )
          .addButton((btn) =>
            btn
              ...
              .onClick(async () => {
                this.plugin.stopWatcher(bridge.id);
                this.plugin.settings.bridges =
                  this.plugin.settings.bridges.filter(
                    (b) => b.id !== bridge.id,
                  );
                await this.plugin.saveSettings();
                this.display();
